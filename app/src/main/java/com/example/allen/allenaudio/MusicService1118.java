 ...
 */
public class MusicService1118 extends MediaBrowserServiceCompat implements MediaPlayer.OnPreparedListener, MediaPlayer.OnCompletionListener, MediaPlayer.OnErrorListener {

    private static final String TAG = MusicService1118.class.getSimpleName();
    ...
    public void onLoadChildren(@NonNull String parentId, @NonNull Result<List<MediaBrowserCompat.MediaItem>> result) {
        ...
        builder1.setMediaUri(Uri.parse("android.resource://" + getPackageName() + "/" + R.raw.lcmw_0));
...
//        mMediaSession.setQueueTitle("Auto Demo");
    }
    ...
    private void pauseMusic() {
        if(mMediaPlayer != null){
            mMediaPlayer.pause();
            mMediaSession.setPlaybackState(new PlaybackStateCompat.Builder().setState(PlaybackStateCompat.STATE_PAUSED, mMediaPlayer.getCurrentPosition(), 0.5f).setActions(PlaybackStateCompat.ACTION_PLAY_PAUSE | PlaybackStateCompat.ACTION_SKIP_TO_NEXT | PlaybackStateCompat.ACTION_SKIP_TO_PREVIOUS).build());
        }
    }
    ...
    private void resumeMusic() {
        if(mMediaPlayer!=null){
            mMediaPlayer.start();
            mMediaSession.setPlaybackState(new PlaybackStateCompat.Builder().setState(PlaybackStateCompat.STATE_PLAYING, mMediaPlayer.getCurrentPosition(), 0.5f).setActions(PlaybackStateCompat.ACTION_PLAY_PAUSE | PlaybackStateCompat.ACTION_SKIP_TO_NEXT | PlaybackStateCompat.ACTION_SKIP_TO_PREVIOUS).build());
        }
    }
...
//            bob.putString(MediaMetadataCompat.METADATA_KEY_ALBUM, mCurrentSongItem != null ? mCurrentSongItem.getAlbumName() : null);
//            bob.putBitmap(MediaMetadataCompat.METADATA_KEY_ALBUM_ART, mCurrentSongItem != null ? albumBitmap : null);
...
//                    .setLargeIcon(b)
                    ...
                    .setContentText((String) mPlayingQueue.get(mCurrentQueueIndex).getDescription().getTitle())
